...
.. @todo: Detailed geometry, fasteners, and CAD files for every part live in the :doc:`parts_catalog` and the per-subsystem pages linked below.
...
.. seealso::
    ...
    :doc:`Frame and Extrusion <subsystems/frame_and_extrusion>` 
        Extrusion layout and main frame geometry

    :doc:`Drive System <subsystems/drivetrain>` 
        Motor → coupling → wheel torque path

    :doc:`Suspension <subsystems/suspension>` 
        Rocker-differential mechanics in detail

.. @todo: - :doc:`subsystems/hull_and_enclosures` — hull, top cover, battery compartment, sensor mounts
.. @todo: - :doc:`fabrication_guide` — step-by-step assembly order
.. @todo: - :doc:`parts_catalog` — every part with files and BOM
.. @todo: - :doc:`hardware_catalog` — fasteners, inserts, bearings