...
Mechanical design and fabrication of the RoboFlock rover. This section covers the design intent for each subsystem, the parts that make it up, the hardware that holds it together, and a step-by-step build order.
...
Each assembly has its own subsystem page; every part across all assemblies is catalogued (with files, hardware BOM, and sourcing) in the :doc:`Parts Catalog <parts_catalog>`.
