...
Fortunately, ROS2 can handle the translational and rotational math needed to move data between different frames of reference via the :code:`tf2` subsystem; however, it still needs a starting point. This is where the URDF file comes in. It gives a spatial description of each component in the robot relative to the robot's center of mass. 

Using the left ultrasonic example, we might describe its translational offset as being 10 centimeters above and 15 centimeters to the left of the robot's center, and its rotational offset would be 90 degrees about the *z*-axis. 

.. The file can be manually written or automatically generated from CAD models using tools such as **LIST CAD-URDF TOOL HERE**.
...
`SLAM toolbox <https://github.com/SteveMacenski/slam_toolbox>`_ is an open-source ROS2 package that uses the robot's odometry and incoming LiDAR scans to build a map of the surroundings, which gets published alongside a corresponding :code:`map` to :code:`odom` transformation. 

.. figure:: ../_images/flowcharts/slam_toolbox_sync.png
    :align: center
    :loading: link
...
Nav2 is ROS2's navigation stack that takes in a set of `REP-105 <https://www.ros.org/reps/rep-0105.html>`_ compliant TF transformations, a BT (behavior tree) XML file, and any relevant sensor data, which in the case of RoboFlock is the IMU, ultrasonic sensor, and GPS data. 

Transformations and sensor data needed by Nav2 is obtained by configuring it to subscribe to the corresponding topics. This data gets used to build several layers of costmaps, allowing the pathfinding behavior to choose the best, obstacle-free route to its goal. The BT XML file is generated by a set of plugins that is defined when configuring Nav2. See the `RoboFlock GitHub repository <https://github.com/shouvik-d/roboflock_ws/blob/main/src/bring_up/config/nav2_params.yaml>`_ for the Nav2 plugins used in this project.


.. seealso::

    `Introducing tf2 <https://docs.ros.org/en/humble/Tutorials/Intermediate/Tf2/Introduction-To-Tf2.html>`_
        ROS2 Humble tutorial on the :code:`tf2` and the :code:`rviz2` visualization tool
